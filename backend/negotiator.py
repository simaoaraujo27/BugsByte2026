...
def negotiate_craving(craving: str, target_calories: int = 600, mood: Optional[str] = None, favorite_recipes: List[schemas.Recipe] = [], allergens: List[str] = []) -> schemas.NegotiatorResponse:
    ...
    prompt = (
        ...
        f"{allergen_context}"
        "\nINSTRUÇÕES DE CUMPRIMENTO ESTRITO:\n"
        "1. É PROIBIDO alterar o prato base. Se o utilizador pediu Sushi, a receita TEM de ser de Sushi. Se pediu Pizza, TEM de ser Pizza. Se pediu Bolo de Iogurte, TEM de ser Bolo de Iogurte.\n"
        "2. A criatividade deve ser aplicada APENAS para tornar o prato pedido mais saudável (ex: usar farinha integral, menos açúcar, técnicas de cozedura mais leves), MAS NUNCA para mudar o tipo de comida.\n"
        "3. Ignora sugestões de 'Estilo' ou 'Inspiração' se estas entrarem em conflito com o prato mandatório pedido pelo utilizador.\n"
        "4. Se o pedido for algo não comestível ou impossível de criar, define 'recipe' como null.\n"
        "5. Responde sempre em PORTUGUÊS DE PORTUGAL (PT-PT).\n"
        "6. PROIBIDO usar quinoa/qinoa.\n"
        f"\nNOTAS ADICIONAIS (Secundárias ao prato mandatório):\n"
